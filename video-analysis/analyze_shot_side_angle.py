...
def detect_ball_position(frame, frame_height, frame_width, previous_ball_pos=None, color_lower=(0,0,200), color_upper=(180,50,255)):
...
# List of videos to process with corresponding output files
video_configs = [
    ("better_ideal_shot.mp4", "ideal", "metrics_ideal_shot.txt"),
    ("test_shot_better.mp4", "test", "metrics_test_shot.txt")
]

# Process each video
for video_path, shot_type, output_file in video_configs:
    # Load video
    ...
    # Open file to save metrics for this video
    with open(output_file, 'w') as f:
        # Write header (no shot_type column since files are separate)
        f.write("frame,hip_x,hip_y,knee_x,knee_y,ankle_x,ankle_y,left_hip_x,left_hip_y,left_knee_x,left_knee_y,plant_foot_x,plant_foot_y,ball_pos_x,ball_pos_y,knee_angle,plant_foot_distance\n")
        ...
        for n in range(total_frames):
            ...
            ball_pos, ball_draw = detect_ball_position(frame_bgr, frame_height, frame_width, previous_ball_pos)
            ...
            if ball_pos is not None and results.pose_landmarks:
                ...
                if all(landmarks[i].visibility > 0.7 for i in required_landmarks):
                    ...
                    print(f"[{shot_type}] Frame {n}, Hip: {hip}, Knee: {knee}, Ankle: {ankle}")

                    # Validate right leg keypoints (y: ankle > knee > hip)
                    if not (ankle[1] > knee[1] > hip[1]):
                        print(f"[{shot_type}] Frame {n}, Warning: Right leg keypoints misaligned, attempting correction")
                        knee = (hip + ankle) / 2
                    ...
                    print(f"[{shot_type}] Frame {n}, Left Hip: {left_hip}, Left Knee: {left_knee}, Plant foot: {plant_foot}, Ball pos: {ball_pos}")

                    # Validate left leg keypoints (y: ankle > knee > hip)
                    if not (plant_foot[1] > left_knee[1] > left_hip[1]):
                        print(f"[{shot_type}] Frame {n}, Warning: Left leg keypoints misaligned, attempting correction")
                    ...
                    print(f"[{shot_type}] Frame {n}, Ball radius: {ball_radius_pixels}px")
                    ...
                    print(f"[{shot_type}] Frame {n}, Alternative distance (player height): {distance_cm_alt:.2f}cm")

                    # Detect contact frame
                    foot = np.array([landmarks[mp_pose.PoseLandmark.RIGHT_FOOT_INDEX].x * frame_width,
                                     landmarks[mp_pose.PoseLandmark.RIGHT_FOOT_INDEX].y * frame_height])
                    foot_to_ball_distance = np.linalg.norm(foot - ball_pos)
                    if foot_to_ball_distance < 50 and collect_metrics:
                        print(f"[{shot_type}] Contact frame detected at frame {n}, stopping metric collection")
                        contact_frame = n
                        collect_metrics = False

                    # Collect and save metrics if before or at contact
                    if collect_metrics:
                        print(f"[{shot_type}] Frame {n}, Knee angle: {knee_angle:.2f}°, Plant foot distance: {distance_cm:.2f}cm")
